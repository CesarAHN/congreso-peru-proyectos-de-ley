...
Se crea este repositorio con el fin de poder conocer cómo están trabajando los congresistas de la república del Perú, específicamente en la presentación de proyectos de ley y en la concreción final de convertirse en ley.

Se muestra información del número de proyectos de ley y leyes por bancadas, regiones y por congresistas. 

Esta información se extrajo desde el portal del congreso de la república  ---> https://www.congreso.gob.pe/

Los códigos lo puede encontrar en el repositorio con el nombre de `script_congreso_pl.R` y el data frame que compila toda la información es `df`.

**IMPORTANTE:**

Los proyectos de ley son presentados por un autor principal, pero a este se pueden unir más congresistas que apoyan el proyecto a los cuales se les llama "coautores". En los gráficos siguientes cuando se menciona la palabra "involucrados" se hace referencia a los congresistas que son autores o coautores en un determinado proyecto de ley, cuando se indica la palabra "autores principales" se hace referencia al congresista que presentó el proyecto de ley lo que implica que él generó el documento que contiene la exposición de motivos. 

Se aclara esta situación, ya que los congresistas que son autores principales son los que en realidad juntamente con sus asesores crean el proyecto de ley, por otra parte, los congresistas que son coautores solo apoyan en la presentación.
...
Todos los congresistas al menos han estado involucrados en la presentación de un proyecto de ley.
...
df %>% group_by(Autores,image,Partido) %>% count() %>% arrange(-n) %>% tail(n=11) %>% 
  ...
  geom_label(aes(x=reorder(Autores,-n),y=n+.7,label=n),show.legend=F, bg="white", size=5)+
  coord_flip()+
  theme_bw()+
  theme(legend.position = "none",
        plot.caption = element_text(face = "bold", size = 7),
...
df %>% group_by(`Proyecto de Ley`) %>% summarise(Autores=first(Autores)) %>% left_join(df) %>% 
  ...
  labs(x="Congresistas",y="Número de proyectos",title = "LOW - 10 CONGRESISTAS COMO PRINCIPALES AUTORES\nEN LA PRESENTACIÓN DE PROYECTOS DE LEY.",
       fill="", caption = paste0("Solo proyectos presentados por el congreso.\n",
                                 "FUENTE: CONGRESO DE LA REPÚBLICA."),
       subtitle=paste0("Congresistas con menos proyectos de Ley.\nActualizado al: ",fecha))+
  geom_label(aes(x=reorder(Autores,-n),y=n+.05,label=n),show.legend=F, bg="white", size=5)+
  coord_flip()+
  theme_bw()+
  theme(legend.position = "none",
        plot.caption = element_text(face = "bold", size = 7),
...
```{r, fig.width=7, fig.height=10, message=FALSE, warning=FALSE, echo=FALSE}
...
Sur_America <- st_read("C:/OTROS/shapefile/sudamerica/Sudamérica.shp")
...
df %>% distinct(`Proyecto de Ley`,Estado, Partido,image) %>% group_by(Partido,image) %>% count(Estado) %>% 
  filter(grepl("AUTOGRAFA|PERUANO|INSISTE|DISPENSA",Estado)) %>% filter(!is.na(Partido)) %>% mutate(sum=sum(n)) %>% 
...
df %>% distinct(`Proyecto de Ley`,Estado, Autores,image,Partido) %>% group_by(Partido,Autores,image) %>% count(Estado) %>% 
  filter(grepl("AUTOGRAFA|PERUANO|INSISTE|DISPENSA",Estado)) %>% filter(!is.na(Autores)) %>% mutate(sum=sum(n)) %>% 
...
```{r,fig.width=10, fig.height=6, echo=F, message=FALSE, warning=FALSE}
df %>% group_by(`Proyecto de Ley`) %>% summarise(Autores=first(Autores)) %>% left_join(df) %>% 
  filter(grepl("AUTOGRAFA|PERUANO|INSISTE|DISPENSA",Estado)) %>% group_by(Autores, image,Partido) %>% count(sort = T) %>% head(n=8) %>% 
  ggplot(aes(x=reorder(Autores,n),y=n, fill=Partido, image=image))+
  geom_isotype_col(img_height = grid::unit(1, "null"), img_width = NULL,
                   ncol = 1, nrow = 1, hjust = 1, vjust = 0.5, colour="black")+
  scale_fill_brewer(palette = "Set1")+
...
```{r, fig.width=12, fig.height=7, echo=F, message=FALSE, warning=FALSE}
df  %>% 
...
```{r, fig.width=12, fig.height=7, echo=F, warning=FALSE, message=FALSE}
df %>%
  distinct(titulo, proyecto_de_ley) %>%
  unnest_tokens(word, titulo, drop = FALSE) %>% 
  distinct(proyecto_de_ley,word) %>%
...
```{r, fig.width=14, fig.height=10, echo=F, warning=FALSE, message=FALSE}
...
```{r, fig.width=14, fig.height=9, echo=F, warning=FALSE, message=FALSE}
for (i in 1:dim(n1)[1]) {
  assign(paste0("pp_",i),
         df %>% filter(autores==n1$autores[i]) %>%
           distinct(titulo,proyecto_de_ley) %>%
...
La primera versión se publicó el día **11/12/2021**, pero en los siguientes días se ha venido actualizando la información, en un principio se pensó hasta el 15 de diciembre, pero el congreso amplió su legislatura por segunda vez y estará sesionando hasta el 02 de febrero de 2022, día que entrará en receso el congreso. 
